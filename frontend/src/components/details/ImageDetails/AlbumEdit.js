...
const AlbumEdit = () => {
  ...
  const handleRemoveFile = (index) => {
    ...
    if (removedFile.file instanceof File) {
      ...
      setFileToDelete((prev) => [...prev, removedFile.url]);
    }
  ...
  const handleAlbumEdit = async (data, selectedFriends, visibility) => {
    setPosting(true);
    setErrMsg('');
    ...
    try {
      ...
      const uploadedFiles = await Promise.all(
        existingFiles.map(async (file) => {
          const uri = await handleFileUpload(file.file);
          return uri;
        }),
      );
      ...
      setErrMsg('An error occurred while editing the album.');
    } finally {
      setPosting(false);
    }
  };
  ...
  useEffect(() => {
    if (existingData) {
      setValue('albumName', existingData.albumName);
      const existingDescriptions = existingData.images.map(
        (image) => image.description,
      );
      setDescriptions(existingDescriptions);
      setFile(
        existingData.images.map((image) => ({
          url: image.url,
        })),
      );
    }
