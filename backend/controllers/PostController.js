...
const deletePost = async (req, res) => {
  try {
    const { id } = req.user
    const existingPost = await PostModel.findByIdAndDelete({ _id: req.params.id, user: id })
    res.json({
...
const checkViewFriend = async (req, res) => {
  ...
  const shareUser = currentUser.friends.filter(friend => viewers.includes(friend));
